                ...
                <div className="flex px-2 flex-row relative">
                    ...
                    <div className="h-full overflow-y-auto px-2">
                        ...
                        { ( products.filter((data)=>{
                                if(search == null)
                                    return data
                                 else if(data.art_des.toString().toLowerCase().includes(search.toLowerCase())){
                                    return data 
                                }
                        ...
                        <div className="grid grid-cols-6 gap-4 pb-3">
                            { products.filter((data)=>{
                                ...
                                <div onClick={() => addToCart(item)} key={item.cod_art} role="button" className="select-none cursor-pointer transition-shadow overflow-hidden rounded-2xl bg-white shadow hover:shadow-lg" >
                                    <div className="w-full h-72">
                                        <img src={item.image} alt={item.art_des} />
                                    </div>
                                    <div className="flex pb-3 px-3 text-sm -mt-3">
                                        <p className="flex-grow truncate mr-1">{ item.art_des}</p>
                                        <p className="nowrap font-semibold">{ priceFormat(item.price) }</p>
                                    </div>
                                </div>
                            ))}
